...
<div class="page-content">
    <div class="container-fluid">
        ...
        <div class="row" id="image-container">
            <div class="col-lg-12">
                <form id="countryForm" method="post" action="{{route('feature.heading.post')}}" enctype="multipart/form-data">
                @csrf
                    <div class="card">
                        <div class="card-header align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">Feature Heading</h4>
                        </div><!-- end card header -->
                        <div class="card-body">
                            <div class="live-preview">
                                <div class="row gy-4">
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'heading', 'label'=>'Heading', 'value'=>!empty($featureHeading) ? (old('heading') ? old('heading') : $featureHeading->heading) : old('heading')])
                                        <p>
                                            <code>Note: </code> Put the text in between span tags to make it highlighted
                                        </p>
                                    </div>
                                    <div class="col-xxl-6 col-md-6">
                                        @include('admin.includes.input', ['key'=>'sub_heading', 'label'=>'Sub Heading', 'value'=>!empty($featureHeading) ? (old('sub_heading') ? old('sub_heading') : $featureHeading->sub_heading) : old('sub_heading')])
                                    </div>
                                    <div class="col-xxl-12 col-md-12">
                                        @include('admin.includes.textarea', ['key'=>'description', 'label'=>'Description', 'value'=>!empty($featureHeading) ? (old('description') ? old('description') : $featureHeading->description) : old('description')])
                                    </div>
                                    <div class="col-xxl-12 col-md-12">
...
.addField('#description', [
  ...
  .onSuccess(async (event) => {
    ...
    try {
        ...
        const response = await axios.post('{{route('feature.heading.post')}}', formData)
